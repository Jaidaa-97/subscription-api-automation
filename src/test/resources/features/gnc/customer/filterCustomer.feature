Business Need: Filter customer
  ...
  Scenario: Search customer by customer name
    Given I have endpoint "/data-subscription/v1/customers?search=Jaidaa Mahmoud Osta"
    And I run get call api
    Then I see response code 200
    Then I see property value "Jaidaa" is contains in the response property "data.customers[0].fullName"

  Scenario: Search customer by status
    Given I have endpoint "/data-subscription/v1/customers?status=INACTIVE,ACTIVE"
    And I run get call api
    Then I see response code 200
